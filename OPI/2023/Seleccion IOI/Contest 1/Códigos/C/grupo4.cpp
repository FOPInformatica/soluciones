...
int main() {
    ...
    for(int i = 0; i < m; i++) {
        ...
        int node = get_node(P);
        if(~node) {
            if(in[node]) pending[in[node] - 1].emplace_back(-(i + 1));
            pending[out[node]].emplace_back(i);
        }
    }
    nodes = 0;
    add_node();
    for(int i = 0; i < n; i++) {
        insert(s[sorted[i]], sorted[i]);
        for(int to : pending[i]) {
            if(to >= 0) ans[to] += query(Q[to]);
            else ans[-to - 1] -= query(Q[-to - 1]);
        }
    }
